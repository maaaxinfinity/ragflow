...
export function ControlPanel({
  ...
  // Desktop default collapsed; mobile sheet always expanded
  useEffect(() => {
    if (isMobile) {
  ...
  return (
    <div
      className={`${isMobile ? 'w-full' : isCollapsed ? 'w-14' : 'w-80'} border-l flex flex-col h-full bg-gradient-to-b from-background to-muted/20 overflow-y-auto relative transition-all duration-300`}
    >
      {!isMobile && (
        <Button
          variant="ghost"
          ...
          className="absolute top-4 -left-3 z-20 h-6 w-6 rounded-full border bg-background shadow-md hover:shadow-lg transition-all"
          title={isCollapsed ? '展开配置面板' : '收起配置面板'}
        >
          {isCollapsed ? (
      ...
      <div className="flex-1 p-4 space-y-6">
        ...
        {!isCollapsed && (currentLoginUser || userInfo || currentUserInfo) && (
          <div className="pt-4 border-t">
