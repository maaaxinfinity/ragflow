...
#### HOTFIX 9.2: 后端POST拒绝null model_card_id

**文件**: `api/apps/free_chat_app.py`  
**位置**: Line 257-267 (POST `/settings`)
...
#### 测试场景2: 拒绝保存null model_card_id
**步骤**:
1. 尝试POST包含null model_card_id的session

**预期结果**:
- ❌ 返回错误码 ARGUMENT_ERROR
- ✅ 错误信息："Invalid session: model_card_id is required for all sessions"
...
**解决方案**: 后端双重防御 + 前端智能合并 + 历史数据迁移  
**预期效果**: 彻底解决输入框消失和会话丢失问题
